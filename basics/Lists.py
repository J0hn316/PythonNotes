...
print(game_cart[:3])   #Output: ['halo', 'pokemon', ' minecraft'] It takes elements from the beginning
print(game_cart[3:])     #Output  : ['fortnite'] It takes all elements starting from a specific index until the end
print(game_cart[0::2])  #Output: ['halo', 'minecraft'] It takes every second element starting from the first one.
print(game_cart[1:3]) #Output: ['pokemon', 'minecraft'] It doesn't take the first element , it starts at the second one.

# When slicing a list it will create a new list in memory.
games = game_cart[1:3] #It creates a new list with two elements, 'pokemon' and 'minecraft'

games = game_cart[:] #  This is called shallow copy, it creates a new reference to the same data 
...
new_li = basket.append(7)  # Returns None because append doesn't return anything to variable
...
basket4.remove(3)  # It removes only one occurrence of the specified value. If there are more than one '3' present then it will remove only one '3'
